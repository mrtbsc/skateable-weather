const DOM = {
    ...
    display : function (array, configuration ) {
            ...
            for (const interval of array) {
                ...
                let dateString = (isNextDays) ? this.stringifyDay(interval.date) : this.stringifyHour(interval.date);
                let rainString = ( isPast? 
                    interval.mmsOfRain + ' mm of rain':  
                    interval.rainProb + '% chance of rain'
                );
                let text = (dateString + '<br>' + rainString + '<br>' + interval.degrees + '°C');
                let newSpan = $('<span>'+ text + '</span>'); 
                let newImg = $('<img>');
                newImg.attr('src', interval.iconPath);
                newSpan.append(newImg);
                
                let container = ( isToday ? $('#today') :
                    (isTomorrow ? $('#tomorrow') :
                        (isNextDays ? $('#week') :
                            $('#yesterday')
                        )
    ...
    stringifyDay : function (d) {
        return `${CONFIG.weekDays[d.getDay()]}<br>${d.getDate()} ${CONFIG.monthNames[d.getMonth()]}`;
    }
